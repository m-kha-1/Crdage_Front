...
export class TaskDetailsComponent {
  ...
  launch(scenePath: string) {
    ...
    this.productionsService.launch_scene(scenePath).subscribe({
      next: (response) => {
  ...
  commentS(i: number, commentaire: string) {
    ...
    this.productionsService
      .CommentSup(this.sharedValue.id, jsonCommentaire)
      .subscribe(
        (comment) => {},
        (error) => {
  ...
  onFileSelected(event: Event, index: number): void {
    ...
    if (file) {
      ...
      reader.onload = (e: any) => {
        ...
        console.log(
          `Data URL for image at index ${index}: ${this.urls[index]}`
        );
      };
  ...
  comment(i: number, commentaire: string) {
    if (localStorage.getItem('userType') == '(supervisor)') {
      ...
      this.jsonNataskTytaskComm = {
        name: this.dataTaskString.name,
        type: this.dataTaskString.type,
        comments_supervisor2: this.dataTaskString.comments_supervisor2,
      };
      console.log(
        'jsonNataskTytaskComm : ',
        this.jsonNataskTytaskComm,
        JSON.stringify(this.jsonNataskTytaskComm)
      );
      const stringNataskTytaskComm = JSON.stringify(this.jsonNataskTytaskComm);
      this.productionsService
        .CommentSup(this.dataTaskString.id, this.jsonNataskTytaskComm)
        .subscribe();
      ...
      if (taskdataCurrentValue != null) {
        ...
        console.log(
          'détails de local storage actuel',
          this.dataTaskString.comments_supervisor2
        );
        taskdataCurrentValueJson.comments_supervisor2 =
          this.dataTaskString.comments_supervisor2;
        console.log(
          'détails de local storage apres modifications',
          JSON.stringify(taskdataCurrentValueJson)
        );
      ...
      alert('You must be a supervisor to make a comment !');
    }
  }
  ...
  ngOnInit() {
    ...
    if (dataTask != null) {
      ...
      console.log(
        'dataTask details:',
        Object.keys(this.dataTaskString.comments_supervisor2)
      );
      console.log(
        'dataTask2 details:',
        Object.values(this.dataTaskString.comments_supervisor2)
      );
    }
    ...
    if (this.organization != null) {
      console.log('organization : ', this.organization);
      this.productionsService
        .listVersionsPublished(
          this.organization,
          this.dataTaskString.production_name,
          this.dataTaskString.type,
          this.dataTaskString.name
        )
        .subscribe((data: any) => {
          this.urls = data;
          console.log('images', this.urls);
        });
    }
